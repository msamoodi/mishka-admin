...
export default function TicketDetailClient({ ticket: initial }: { ticket: Ticket }) {
  ...
  const setStatus = async (status: string) => {
    setResolving(true)
    const res = await fetch(withBasePath("/api/admin/resolve-ticket"), {
      method: "POST",
  ...
  const handleSaveReply = async () => {
    ...
    setSaving(true)
    setError("")
    const res = await fetch(withBasePath("/api/admin/reply-ticket"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
        <Link href="/tickets" className="hover:text-gray-700 transition-colors">Tickets</Link>
        <span>/</span>
