...
const SEGMENT_FILTERS: Record<string, (now: Date) => { column: string; from?: string; to?: string } | null> = {
  ...
  new_users:    (now) => ({ column: "created_at",   from: new Date(now.getTime() - 7 * 86400000).toISOString() }),
}

export async function POST(req: NextRequest) {
  const { headline, description, cta_url, cta_label, segments: rawSegments } = await req.json()
  ...
  const { data: notification, error: notifError } = await supabase
    .from("notifications")
    .insert({ headline: headline.trim(), description: description?.trim() || null, cta_url: cta_url?.trim() || null, cta_label: cta_label?.trim() || null, segment: segmentStr })
    .select("id")
    .single()

  if (notifError || !notification) return NextResponse.json({ error: notifError?.message ?? "Failed to create notification" }, { status: 500 })

  // Fetch target users — union across all selected segments
  const userIdSet = new Set<string>()

  for (const seg of segments) {
    const filter = SEGMENT_FILTERS[seg]?.(now) ?? null
    ...
    ;(users ?? []).forEach((u: { id: string }) => userIdSet.add(u.id))
  }
