...
type Props = {
  ...
  placeholder?: string
  /** Subfolder within videos/, e.g. "courses/product-design/advanced-ux-strategy" */
  folder?: string
}

export default function VideoUploader({ value, onChange, placeholder = "https://… or paste a path", folder }: Props) {
  ...
  const handleFile = async (file: File | undefined) => {
    ...
    setUploading(true)
    try {
      const url = await uploadMedia(file, "videos", folder)
      onChange(url)
    } catch (err) {
