...
function getContents(allFiles: string[], currentPath: string[]) {
  ...
  for (const f of allFiles) {
    const relative = f.replace(/^\/audio\//, "")
    if (prefix) {
...
export default function FolderAudioPicker({ value, onChange, placeholder = "/audio/…" }: Props) {
  ...
  // Stop preview when closed or value changes
  useEffect(() => {
  ...
  const togglePreview = () => {
    ...
    if (playing) {
      ...
      const audio = new Audio(`/api/admin/audio?path=${encodeURIComponent(value)}`)
        ...
        <div className="absolute top-full left-0 z-50 mt-1 w-full min-w-[300px] bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="overflow-y-auto max-h-64 p-2">
            ...
            {!loading && folders.length === 0 && files.length === 0 && (
              <p className="text-xs text-gray-400 text-center py-6">No audio files found</p>
            )}
